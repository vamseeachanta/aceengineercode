...
def setupApplicationRuns(func):
    ...
    def wrapper_applicationRuns(*args, **kwargs):
        ...
        end_time = time.perf_counter()
        run_time = end_time - start_time
        if cfg.default.__contains__('data_source') and cfg.default['data_source'] == 'db':
            save_results.saveApplicationResultsToDB(cfg=kwargs['cfg'], run_time=run_time, run_dict=None)
        ...
        if cfg.default.__contains__('data_source') and cfg.default['data_source'] == 'db':
            ...
            if (run_df is not None) and (len(run_df) > 0):
                for row_index in range(0, len(run_df)):
                    ...
                    end_time = time.perf_counter()
                    run_time = end_time - start_time
                    save_results.saveApplicationResultsToDB(cfg=kwargs['cfg'], run_time=run_time, run_dict=run_dict)
...
class ApplicationRuns():
    ...
    def addRunRecordsToDB(self):
        ...
        if self.cfg.__contains__('db'):
            ...
            run_file_updated = os.path.join("tests\\cfg\\", self.basename, 'runs_added_to_db.csv')
...
class ConfigureApplicationInputs():
    ...
    def unify_application_and_default_and_custom_yamls(self, run_dict):
        self.ApplicationInputFile = 'data_manager\\' + self.basename + '.yml'
        ...
        if run_dict is None:
            try:
                if (sys.argv[1] != None):
                    ...
                    print("Updating default values with contents in file {0}".format(self.customYaml))
            except:
            ...
            self.customYaml = None
            self.CustomInputs = run_dict['CustomInputs']
    ...
    def get_application_configuration_parameters(self, run_dict=None):
        ...
        self.cfg = update_deep_dictionary(self.cfg, application_configuration_parameters)

    def configure_overwrite_filenames(self):
        if self.cfg['default']['config']['overwrite']['output'] == True:
            self.cfg['Analysis']['file_name'] = self.cfg['Analysis']['file_name_for_overwrite']
        try:
            if self.cfg['Analysis']['fe_folder'] == None:
                self.cfg['Analysis']['fe_folder'] = self.cfg['Analysis']['result_folder']
        except:
            pass
...
class SaveApplicationResults():
    ...
    def saveApplicationResultsToDB(self, cfg, run_time, run_dict=None):
        ...
        try:
            ...
            columns = [
                'ApplicationId', 'ProjectId', 'RunName', 'RunStatus', 'DefaultInputFile', 'CustomInputFile',
                'CustomInputs', 'AnalysisTime', 'RunTimeInSeconds'
            ]
            ...
            ProjectIDQuery = "SELECT TOP 1 ProjectId from [dbo].[Project] WHERE ProjectName = '{}'".format(ProjectName)
            ...
            if run_dict is None:
                if CustomInputFile is not None:
                    with open(CustomInputFile, 'r') as ymlfile:
                        try:
                            CustomInputs = json.dumps(yaml.load(ymlfile, Loader=yaml.Loader), default=str)
                        except:
                            CustomInputs = None
                else:
                    CustomInputs = None
            else:
                CustomInputs = run_dict['CustomInputs'].replace("\\\'", "\'").replace("\\n", "\n")
            ...
            ApplicationRunQuery = """
                ...
                IF @@rowcount = 0
                    ...
                    END
                COMMIT TRAN
                """.format(RunStatus, RunTimeInSeconds, ApplicationId, ProjectId, RunName, DefaultInputFile,
