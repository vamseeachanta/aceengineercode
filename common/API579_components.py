class API579_components():
    ...
    def gml(self):
        ...
        for fileIndex in range(0, len(self.gml_wt_grids)):
            ...
            FileName = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + 'Summary_GML_' + str(
                fileIndex)
            print(FileName)
            Acceptable_FCA = np.interp(self.cfg['Design'][0]['InternalPressure']['Outer_Pipe'],
                                       GMLMAWPResultsDF['MAWP'].iloc[::-1].values,
                                       GMLMAWPResultsDF['FCA'].iloc[::-1].values)
            print("Acceptable GML FCA is : {0}".format(Acceptable_FCA))

            self.plot_and_save_gml_results(Acceptable_FCA, GMLMAWPResultsDF, fileIndex)

    def plot_and_save_gml_results(self, Acceptable_FCA, GMLMAWPResultsDF, fileIndex):
        import pandas as pd

        from dataManager.API579.DataFrame_To_Image import DataFrame_To_Image
        from results.API579.plotCustom import plotCustom
        from results.API579.saveData import saveDataYaml
        ...
        file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_' + custom_data[
            'Label'].replace(' ', '_') + '_summary_GML_PLT'

        custom_plot_data = {"PltSupTitle": self.cfg['PlotSettings']['GML']['PltSupTitle'],
                      "PltTitle": self.cfg['PlotSettings']['GML']['PltTitle'] + ', ' + custom_data['Label'],
                      "PltXLabel": self.cfg['PlotSettings']['GML']['PltXLabel'],
                      "PltYLabel": self.cfg['PlotSettings']['GML']['PltYLabel'],
                      'Label': ['MAWP'],
                      'PltColumns': ['FCA', 'MAWP'],
                      "YLim": self.cfg['PlotSettings']['GML']['ylim'],
                      'Axhline': [self.cfg['Design'][0]['InternalPressure']['Outer_Pipe']],
                      'TextFields': [{"x": 0.0, "y": GMLMAWPResultsDF['MAWP'][0] + 100,
                                      "Text": 'MAWP at 0 inch FCA: {0:.0f} psi'.format(
                                          GMLMAWPResultsDF['MAWP'][0])},
                                     {"x": Acceptable_FCA,
                                      "y": self.cfg['Design'][0]['InternalPressure']['Outer_Pipe'] + 150,
                                      "Text": 'Allowable FCA: {0:.4f} inch'.format(Acceptable_FCA)}],
                      'FileName': file_name + '.png'
                      }

        plotCustom(GMLMAWPResultsDF, custom_plot_data)
        file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_' + custom_data[
            'Label'].replace(' ', '_') + '_summary_GML_TBL'
        saveDataYaml(self.cfg, file_name)
        GMLSummaryDF = pd.DataFrame.from_dict(self.cfg['Result']['Circumference'])
        GMLSummaryDF = GMLSummaryDF.round(3)
        cols = ['Description', "Len (inch)", "Min WT (inch)", "Avg. WT (inch)", 'Corr. Rate (inch/year)',
                'Rem. Life (yrs)', "Max WT (inch)"]
    ...
    def prepare_contour_plot_gml(self, df, fileIndex):
        ...
        from results.API579.plotContourf import plotContourf

        if self.cfg.default['Analysis']['GML']['data_source'] == 'xlsx':
            custom_data = self.cfg['ReadingSets'][fileIndex].copy()
        elif self.cfg.default['Analysis']['GML']['data_source'] == 'simulated':
            custom_data  = self.cfg['gml_simulated_grid'][fileIndex].copy()
        file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_' + custom_data['Label'].replace(' ', '_')

        # Generate Heat Map (X = Circumference, Y = Length)
        custom_plot_data = {"Index_Name": 'Length',
                      "Columns_Name": 'Circumference',
                      "FileName": file_name,
                      "PltSupTitle": self.cfg['PlotSettings']['Data']['PltSupTitle'],
                      "PltTitle": self.cfg['PlotSettings']['Data']['PltTitle'] + ' ' +
                                  custom_data['Label'] + ', Tnom: {0} inch, TminBurst: {1} inch'.format(
                          self.cfg.Outer_Pipe['Geometry']['Design_WT'], self.cfg.Outer_Pipe['Geometry']['tmin']),
                      "PltXLabel": self.cfg['PlotSettings']['Data']['PltXLabel'],
                      "PltYLabel": self.cfg['PlotSettings']['Data']['PltYLabel'],
                      "PltLims": custom_data['Contour'],
        ...
        custom_plot_data  = {"Index_Name": 'Length',
                      "Columns_Name": 'Circumference',
                      "FileName": file_name + '_Transpose',
                      "PltSupTitle": self.cfg['PlotSettings']['Data']['PltSupTitle'],
                      "PltTitle": self.cfg['PlotSettings']['Data']['PltTitle'] + ' ' +
                                  custom_data['Label'] + ', Tnom: {0} inch, TminBurst: {1} inch'.format(
                          self.cfg.Outer_Pipe['Geometry']['Design_WT'], self.cfg.Outer_Pipe['Geometry']['tmin']),
                      "PltXLabel": self.cfg['PlotSettings']['Data']['PltYLabel'],
                      "PltYLabel": self.cfg['PlotSettings']['Data']['PltXLabel'],
                      "PltLims": custom_data['Contour'],
                      # https://matplotlib.org/api/_as_gen/matplotlib.pyplot.imshow.html
                      "PltOrigin": 'upper',
                      "PltShowContourLines": False,
    ...
    def lml(self):
        ...
        from dataManager.API579.DataFrame_To_Image import DataFrame_To_Image
        from dataManager.API579.DataFrame_To_xlsx import \
            DataFrameArray_To_xlsx_openpyxl
        ...
        for fileIndex in range(0, len(self.cfg['LML']['LTA'])):
            customdata = {"io": self.cfg['LML']['LTA'][fileIndex]['io'],
            ...
            FileName = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + 'Summary_LML_' + str(
                fileIndex)
            LMLFileNameArray.append('LML_' + str(fileIndex) + '_')
            decimalArray = pd.Series([2, 2, 0, 0, 0, 0, 3, 3, 3, 3, 3, 1, 1, 3, 3, 3],
            ...
            print(FileName)
            Acceptable_FCA = np.interp(self.cfg['Design']['InternalPressure'],
                                       LMLSummaryDF['MAWPr, L2'].iloc[::-1].values,
                                       LMLSummaryDF['FCA'].iloc[::-1].values)
            print("Acceptable LML FCA is : {0}".format(Acceptable_FCA))

            customdata = {"PltSupTitle": self.cfg['PlotSettings']['LML']['PltSupTitle'],
                          "PltTitle": self.cfg['PlotSettings']['LML']['PltTitle'] + ', ' + self.cfg['LML']['LTA'][fileIndex][
                              'Label'],
                          "PltXLabel": self.cfg['PlotSettings']['LML']['PltXLabel'],
                          "PltYLabel": self.cfg['PlotSettings']['LML']['PltYLabel'],
                          'Label': ['MAWP, No Flaw', 'MAWP Reduced, L1', 'MAWP Reduced, L2'],
                          'PltColumns': ['FCA', 'MAWP', 'MAWPr, L1', 'MAWPr, L2'],
                          "YLim": [10000, 20000],
                          'Axhline': [self.cfg['Design']['InternalPressure']],
                          ...
                          'TextFields': [{"x": 0.05, "y": 1200, "Text": "MAWP Reduced"},
                                         {"x": 0.15, "y": 14000,
                                          "Text": "Flaw dimension (inch): s={0} and c={1}".format(
                                              LMLSummaryDF.iloc[0]['s'], LMLSummaryDF.iloc[0]['c'])},
                                         {"x": 0.15, "y": 13500,
                                          "Text": "Nominal WT (inch) ={0}".format(LMLSummaryDF.iloc[0]['tavg'])},
                                         {"x": 0.15, "y": 13000, "Text": "Avg. Measured WT (inch) ={0}".format(
                                             LMLSummaryDF.iloc[0]['tavg'])},
                                         {"x": 0.15, "y": 12000, "Text": "Min. Measured WT (inch)={0}".format(
                                             LMLSummaryDF.iloc[0]['tmm'])},
                                         {"x": 0.15, "y": 11000,
                                          "Text": "FCA outsideFlaw = {0} times FCA flaw".format(
                                              self.cfg['LML']['LTA'][fileIndex]['FCANonFlawRatio'])}
                                         ],
                          'FileName': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                              'file_name'] + 'Summary_LML_' + str(fileIndex) + '_PLT' + '.png'
                          }
            ...
            customdata = {"PltSupTitle": self.cfg['PlotSettings']['LML']['PltSupTitle'],
                          ...
                          'FileName': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                              'file_name'] + 'Summary_LML_' + str(fileIndex) + '_PLT1' + '.png'
                          }
            plotCustom(LMLSummaryDF, customdata)

        #  Send LML Data to Excel Sheet
        customdata = {"FileName": self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '.xlsx',
    ...
    def b31g(self):
        ...
        for fileIndex in range(0, len(self.cfg['B31G'])):
            customdata = {"io": self.cfg['B31G'][fileIndex]['io'],
            ...
            customdata = {"PltSupTitle": self.cfg['PlotSettings']['B31G']['PltSupTitle'],
                          "PltTitle": self.cfg['PlotSettings']['B31G']['PltTitle'] + ', ' + self.cfg['B31G'][fileIndex][
                              'Label'],
                          "PltXLabel": self.cfg['PlotSettings']['B31G']['PltXLabel'],
                          "PltYLabel": self.cfg['PlotSettings']['B31G']['PltYLabel'],
                          ...
                          'FileName': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                              'file_name'] + 'B31G_' + str(fileIndex) + '_PLT' + '.png'
                          }
    ...
    def simulate_wt_grid(self, data):
        ...
        columns = list(range(0, int(self.cfg.Outer_Pipe['Geometry']['Nominal_OD']*math.pi)))
        df = pd.DataFrame(self.cfg.Outer_Pipe['Geometry']['Design_WT']*data['wt_to_nominal_outside_loss_area'], index=index, columns=columns)
    ...
    def read_gml_grids_from_excel(self):
        ...
        for fileIndex in range(0, len(self.cfg['ReadingSets'])):
            customdata = {"io": self.cfg['ReadingSets'][fileIndex]['io'],
    ...
    def API579GML(self, df, fileIndex):
        ...
        cfg = self.cfg
        customdata = {"tmin": cfg.Outer_Pipe['Geometry']['tmin'],
                      "NominalID": cfg.Outer_Pipe['Geometry']['Nominal_OD'] - 2 * cfg.Outer_Pipe['Geometry']['Design_WT'],
                      "RSFa": cfg['API579Parameters']['RSFa'],
                      "NominalWT": cfg.Outer_Pipe['Geometry']['Design_WT'],
                      "Age": cfg['API579Parameters']['Age'],
                      "FCARateFloor": cfg['API579Parameters']['FCARateFloor'],
                      "FutureCorrosionRate": self.cfg.default['Analysis']['GML']['FCARate']
                      }

        # Perform AssementLengthEvaluation for all data
        data = self.GMLAssessmentLengthEvaluation(df.min(axis=None, skipna=True), customdata)
        ...
        if cfg['default']['Analysis']['GML']['Circumference']:
            ...
            if (cfg.default['settings']['AssessmentLengthCeilingFactor_Circumference'] != None):
            ...
            AveragingIndexRange = self.get_nearest_vector_index(df.columns, data['AveragingLength'])
            ...
            for MarchingLengthIndex in range(0, len(df)):
                for MarchingCircumIndex in range(0, len(df.columns)):
                    ...
                    AveragingIndexRange2 = self.get_nearest_vector_index(df.columns, data['AveragingLength'])
                    ...
                    if (cfg.default['settings']['AssessmentLengthCeilingFactor_Circumference'] != None):
                        ...
                        if data['AveragingLength'] > LengthCeilLimit:
                            data['AveragingLength'] = LengthCeilLimit
                    elif (data['AveragingLength'] > math.pi * cfg.Outer_Pipe['Geometry']['Nominal_OD']):
                        data['AveragingLength'] = math.pi * cfg.Outer_Pipe['Geometry']['Nominal_OD']
                    ...
                    HistoricalCorrosionRate_df.iloc[MarchingLengthIndex, ColumnIndex] = customdata[
                        'HistoricalCorrosionRate']
            ...
            PrefixFileName = cfg['Analysis']['result_folder'] + cfg['Analysis']['file_name'] + '_Circumference_' + str(
                fileIndex) + '_'
            self.SaveResults(RemainingLife_df, PrefixFileName + 'RemainingLife_df')
            self.SaveResults(FutureCorrosionRate_df, PrefixFileName + 'FutureCorrosionRate_df')
            self.SaveResults(HistoricalCorrosionRate_df, PrefixFileName + 'HistoricalCorrosionRate_df')
            ...
            self.SaveResults(WTAcceptabilityRatio_df, PrefixFileName + 'WTAcceptabilityRatio_df')
            self.SaveResults(AveragingLength_df, PrefixFileName + 'AveragingLength_df')

            if self.cfg.default['Analysis']['GML']['data_source'] == 'xlsx':
                custom_grid_data = self.cfg['ReadingSets'][fileIndex].copy()
            elif self.cfg.default['Analysis']['GML']['data_source'] == 'simulated':
                custom_grid_data  = self.cfg['gml_simulated_grid'][fileIndex].copy()

            file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_' + custom_grid_data[
                'Label'].replace(' ', '_')

            summary = {"Description": custom_grid_data['Label'],
            ...
            GMLMAWPResults = []
            for FCAIndex in range(0, len(self.cfg.default['Analysis']['GML']['FCA'])):
    ...
    def LMLAcceptability(self, df, cfg, fileIndex):
        ...
        customdata = {"Index_Name": 'Length',
                      "Columns_Name": 'Circumference',
                      "FileName": 'results\\API579\\Plots\\Flaw_' +
                                  cfg['LML']['LTA'][fileIndex]['io'].split('\\')[1].split('.')[0] + '_' +
                                  cfg['LML']['LTA'][fileIndex]['Label'],
                      "PltSupTitle": cfg['PlotSettings']['Data']['PltSupTitle'],
                      "PltTitle": cfg['PlotSettings']['Data']['PltTitle'] + ' ' + cfg['LML']['LTA'][fileIndex][
                          'Label'] + ', Tnom: {0} inch, Tmin: {1} inch'.format(cfg.Outer_Pipe['Geometry']['Design_WT'],
        ...
        # plotHeatMap(df, customdata)
        self.plotContourf(dfLTA, customdata)
        ...
        for FCAIndex in range(0, len(cfg['LML']['LTA'][fileIndex]['FCA'])):
            ...
            LongitudinalFlawLengthParameter = 1.285 * s / math.sqrt(cfg.Outer_Pipe['Geometry']['Nominal_ID'] * tc)

            if (Rt >= 0.2) and (tmm - FCA >= 0.05) and (
                    cfg['LML']['LTA'][fileIndex]['Lmsd'] >= 1.8 * math.sqrt(cfg.Outer_Pipe['Geometry']['Nominal_ID'] * tc)):
            ...
            if MAWP != None:
                customData = {"LongitudinalFlawLengthParameter": LongitudinalFlawLengthParameter,
                              ...
                              "MAWP": MAWP,
                              "tc": tc,
                              "NominalID": cfg.Outer_Pipe['Geometry']['Nominal_ID']
                              }
                ...
                for L2EvalIndex in range(0, len(tavgiL2)):
                    ...
                    LongitudinalFlawLengthParameter = 1.285 * sL2[L2EvalIndex] / math.sqrt(
                        cfg['Geometry']['NominalID'] * tc)
                    customData.update(
                        {"AARatio": AARatio, "LongitudinalFlawLengthParameter": LongitudinalFlawLengthParameter})
    ...
    def LMLMAWPrEvaluation(self, data, cfg, Level):
        ...
        if data['LongitudinalFlawLengthParameter'] <= 0.354:
            ...
            Mt = np.interp(data['LongitudinalFlawLengthParameter'],
