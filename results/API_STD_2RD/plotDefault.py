...
def plotDefault(cfg, dataDF):
    ...
    plt.suptitle(cfg['plotSettings']['variableWTBurst']['pltSupTitle'], fontsize=14, color='black')
    #it creates plot Sub-tittle, color and size
    plt.title(cfg['plotSettings']['variableWTBurst']['pltTitle'], fontsize=12, color='black')
    #it creates plot X-axis name, fontsize and colour
    plt.xlabel(cfg['plotSettings']['variableWTBurst']['pltXLabel'], fontsize=12,  color='black')
    #it creates plot Y-axis name, fontsize and colour
    plt.ylabel(cfg['plotSettings']['variableWTBurst']['pltYLabel'], fontsize=12, color='black')
    #plotting x and y values
    #plt.plot(x_axis, y_axis1, '-b', label='BurstPressure')
    plt.plot(dataDF["t"], dataDF["Pd"], '-b', label=cfg['plotSettings']['variableWTBurst']['label'])
    plt.ylim(cfg['plotSettings']['variableWTBurst']['yLim'])
    ...
    plt.grid()
    plt.savefig('results\\API_STD_2RD\\'+cfg['plotSettings']['variableWTBurst']['plotFileName'], dpi=800)
    plt.close()
