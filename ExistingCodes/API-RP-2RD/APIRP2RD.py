...
from dataManager.API_RP_2RD.loadConfiguration import loadConfiguration

configParams = loadConfiguration("dataManager\API_RP_2RD\APIRP2RD.ini")

def APIRP2RDCalculation(APIRP2RD):
    from custom.API_RP_2RD.APIRP2RD_Methods import (APIRP1, APIRP2, APIRP3,
                                                    APIRP4, APIRP5, APIRP6,
                                                    APIRP7, APIRP8)
    ...
    plt.savefig("results\\APIRP2RD\\Tension vs Moment.png",dpi=800)

    plt.xlim(0, 1.1*max(x3))
    plt.ylim(0, 1.1*max(y1))
    plt.plot(x_axis, y_axis)
    plt.savefig("results\\APIRP2RD\\Tension vs Moment_Quad1.png",dpi=800)
