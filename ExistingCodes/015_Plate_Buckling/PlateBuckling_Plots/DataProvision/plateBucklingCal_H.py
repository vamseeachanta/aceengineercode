...
σ_e1=sqrt(σH_xx**2+σH_yy**2-(σH_yy*σH_xx)+(3*τ_H**2))  # Vonmises Stress (σe)
...
# Reduced Slenders ratio # σH_xx,σH_yy,τ_H 
λx_simp=round(sqrt(σ_kx/σExx_Simp),2)
λy_simp=sqrt(σ_ky/σEyy_Simp)
λτ_simp=sqrt(τ_k/τE_simp)
λe_simp=sqrt(f_I/σ_e1*((σH_xx/σExx_Simp)**c+(σH_yy/σEyy_Simp)**c+(τ_H/τE_simp)**c)**(1/c))


# Characteristic Buckling Resistance for serviceability
σscrxH_simp=σ_kx/sqrt(1+λx_simp**4)
σscryH_simp=σ_ky/sqrt(1+λy_simp**4)
σscrzH_simp=τ_k/sqrt(1+λτ_simp**4)
σescrH_simp=f_I/sqrt(1+λe_simp**4)


# Usage factor for serviceability check, Simply Supported
ηsxH_simp=σH_xx/σscrxH_simp
FALSEH=σH_yy/σscryH_simp
ηszH_simp=τ_H/σscrzH_simp
ηseH_simp=σ_e1/σescrH_simp


# Characteristic Buckling Resistance for Ultimate check
σucrxH_simp1=(σ_kx/(sqrt(1+λx_simp**4)))
σucrx_simp2=σ_kx/sqrt(2)/λx_simp
if(λx_simp<1):
    print("The value of σucrx_simp1 is ",σucrxH_simp1)
else:
    print("The value of σucrx_simp2 is",σucrx_simp2)
σucry_simp1=(σ_ky/(sqrt(1+λy_simp**4)))
σucryH_simp2=σ_ky/sqrt(2)/λy_simp
if(λy_simp<1):
    print("The value of σucry_simp1 is ",σucry_simp1)
else:
    print("The value of σucry_simp2 is",σucryH_simp2)
σucrzH_simp1=(τ_k/(sqrt(1+λτ_simp**4)))
σucrz_simp2=τ_k/sqrt(2)/λτ_simp
if(λτ_simp<1):
    print("The value of σucrz_simp1 is ",σucrzH_simp1)
else:
    print("The value of σucrz_simp2 is",σucrz_simp2)
σeucr_simp1=(σ_e/(sqrt(1+λe_simp**4)))
σeucrH_simp2=σ_e/sqrt(2)/λe_simp
if(λe_simp<1):
    print("The value of σeucr_simp1 is ",σeucr_simp1)
else:
    print("The value of σeucr_simp2 is",σeucrH_simp2)

# Usage factor for ultimate check, , Simply Supported
ηuxH_simp=σH_xx/σucrxH_simp1
ηuyH_simp=σH_yy/σucryH_simp2
ηuzH_simp=τ_H/σucrzH_simp1
ηueH_simp=σ_e1/σeucrH_simp2
...
λe_side=sqrt(f_I/σ_e1*((σH_xx/σExx_Simp)**c+(σH_yy/σEyy_Simp)**c+(τ_H/τE_Simp)**c)**(1/c))


# Characteristic Buckling Resistance for serviceability
σscrxH_side=σ_kx/sqrt(1+λx_side**4)
σscryH_side=σ_ky/sqrt(1+λy_side**4)
σscrzH_side=τ_k/sqrt(1+λτ_side**4)
σescrH_side=f_I/sqrt(1+λe_side**4)


# Usage factor for serviceability check, Sides Clamped
ηsxH_side=σH_xx/σscrxH_side
ηsyH_side=σH_yy/σscryH_side
ηszH_side=τ_H/σscrzH_side
ηseH_side=σ_e1/σescrH_side


# Characteristic Buckling Resistance for Ultimate Check
σucrxH_side1=σ_kx/(sqrt(1+λx_side**4))
σucrx_side2=σ_kx/sqrt(2)/λx_side
if(λx_side<1):
    print("The value of σucrx_side1 is ",σucrxH_side1)
else:
    print("The value of σucrx_side2 is",σucrx_side2)
σucry_side1=σ_ky/(sqrt(1+λy_side**4))
σucryH_side2=σ_ky/sqrt(2)/λy_side
if(λy_side<1):
    print("The value of σucry_side1 is ",σucry_side1)
else:
    print("The value of σucry_side2 is",σucryH_side2)
σucrzH_side1=τ_k/(sqrt(1+λτ_side**4))
σucrz_side2=τ_k/sqrt(2)/λτ_side
if(λτ_side<1):
    print("The value of σucrz_side1 is ",σucrzH_side1)
else:
    print("The value of σucrz_side2 is",σucrz_side2)
σeucr_side1=σ_e/(sqrt(1+λe_side**4))
σeucrH_side2=σ_e/sqrt(2)/λe_side
if(λe_side<1):
    print("The value of σeucr_side1 is",σeucr_side1)
else:
    print("The value of σeucr_side2 is",σeucrH_side2)


# Usage factor for ultimate check, Sides Clamped
ηuxH_side=σH_xx/σucrxH_side1
ηuyH_side=σH_yy/σucryH_side2
ηuzH_side=τ_H/σucrzH_side1
ηueH_side=σ_e1/σeucrH_side2
...
k3=1/(2*λ_c**2)+0.07
p_Sd_pa=101325+1025*D_H*x9
p_Sd_ksi=p_Sd_pa*x10*x11
x12= 2*(x3**2)*f_I       #x7=2*(t_H/s_H)^2*f_y
...
σy_R=(1.3*t_H/l_H*sqrt(E_I/f_I)+k4_I*(1-1.3*t_H/l_H*sqrt(E_I/f_I)))*f_I*Kp1
σy_rd=σy_R/ϒ_M
...
σH_yrd=σy_rd
τ_rdH=τrd
x15=(σH_xx/σH_xrd)**2+(σH_yy/σH_yrd)**2-ci_2*(σH_yy/σH_xrd)*(σH_yy/σH_yrd)+(τ_H/τ_rdH)**2
...
LongitudinalH=σH_xx/σxrd
TransverseH=σH_yy/σy_rd
ShearH=τ_H/τ_rd
BiaxialH=sqrt(x15)
