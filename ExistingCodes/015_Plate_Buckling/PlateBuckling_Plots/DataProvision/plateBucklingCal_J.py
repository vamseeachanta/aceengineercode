...
σscrxJ_simp=σ_kx/sqrt(1+λx_simp**4)
σscryJ_simp=σ_ky/sqrt(1+λy_simp**4)
σscrzJ_simp=τ_k/sqrt(1+λτ_simp**4)
σescrJ_simp=f_I/sqrt(1+λe_simp**4)


# Usage factor for serviceability check, Simply Supported
ηsxJ_simp=σJ_xx/σscrxJ_simp
FALSEJ=σJ_yy/σscryJ_simp
ηszJ_simp=τ_J/σscrzJ_simp
ηseJ_simp=σ_e1/σescrJ_simp


# Characteristic Buckling Resistance for Ultimate check
σucrxJ_simp1=(σ_kx/(sqrt(1+λx_simp**4)))
σucrx_simp2=σ_kx/sqrt(2)/λx_simp
if(λx_simp<1):
    print("The value of σucrx_simp1 is ",σucrxJ_simp1)
else:
    print("The value of σucrx_simp2 is",σucrx_simp2)
σucry_simp1=(σ_ky/(sqrt(1+λy_simp**4)))
σucryJ_simp2=σ_ky/sqrt(2)/λy_simp
if(λy_simp<1):
    print("The value of σucry_simp1 is ",σucry_simp1)
else:
    print("The value of σucry_simp2 is",σucryJ_simp2)
σucrzJ_simp1=(τ_k/(sqrt(1+λτ_simp**4)))
σucrz_simp2=τ_k/sqrt(2)/λτ_simp
if(λτ_simp<1):
    print("The value of σucrz_simp1 is ",σucrzJ_simp1)
else:
    print("The value of σucrz_simp2 is",σucrz_simp2)
σeucr_simp1=(σ_e/(sqrt(1+λe_simp**4)))
σeucrJ_simp2=σ_e/sqrt(2)/λe_simp
if(λe_simp<1):
    print("The value of σeucr_simp1 is ",σeucr_simp1)
else:
    print("The value of σeucr_simp2 is",σeucrJ_simp2)

# Usage factor for ultimate check, , Simply Supported
ηuxJ_simp=σJ_xx/σucrxJ_simp1
ηuyJ_simp=σJ_yy/σucryJ_simp2
ηuzJ_simp=τ_J/σucrzJ_simp1
ηueJ_simp=σ_e1/σeucrJ_simp2
...
# Characteristic Buckling Resistance for serviceability
σscrxJ_side=σ_kx/sqrt(1+λx_side**4)
σscryJ_side=σ_ky/sqrt(1+λy_side**4)
σscrzJ_side=τ_k/sqrt(1+λτ_side**4)
σescrJ_side=f_I/sqrt(1+λe_side**4)


# Usage factor for serviceability check, Sides Clamped
ηsxJ_side=σJ_xx/σscrxJ_side
ηsyJ_side=σJ_yy/σscryJ_side
ηszJ_side=τ_J/σscrzJ_side
ηseJ_side=σ_e1/σescrJ_side


# Characteristic Buckling Resistance for Ultimate Check
σucrxJ_side1=σ_kx/(sqrt(1+λx_side**4))
σucrx_side2=σ_kx/sqrt(2)/λx_side
if(λx_side<1):
    print("The value of σucrx_side1 is ",σucrxJ_side1)
else:
    print("The value of σucrx_side2 is",σucrx_side2)
σucry_side1=σ_ky/(sqrt(1+λy_side**4))
σucryJ_side2=σ_ky/sqrt(2)/λy_side
if(λy_side<1):
    print("The value of σucry_side1 is ",σucry_side1)
else:
    print("The value of σucry_side2 is",σucryJ_side2)
σucrzJ_side1=τ_k/(sqrt(1+λτ_side**4))
σucrz_side2=τ_k/sqrt(2)/λτ_side
if(λτ_side<1):
    print("The value of σucrz_side1 is ",σucrzJ_side1)
else:
    print("The value of σucrz_side2 is",σucrz_side2)
σeucr_side1=σ_e/(sqrt(1+λe_side**4))
σeucrJ_side2=σ_e/sqrt(2)/λe_side
if(λe_side<1):
    print("The value of σeucr_side1 is",σeucr_side1)
else:
    print("The value of σeucr_side2 is",σeucrJ_side2)


# Usage factor for ultimate check, Sides Clamped
ηuxJ_side=σJ_xx/σucrxJ_side1
ηuyJ_side=σJ_yy/σucryJ_side2
ηuzJ_side=τ_J/σucrzJ_side1
ηueJ_side=σ_e1/σeucrJ_side2
...
σJ_yrd=σy_rd
τ_rdJ=τrd
x15=(σJ_xx/σJ_xrd)**2+(σJ_yy/σJ_yrd)**2-ci_2*(σJ_yy/σJ_xrd)*(σJ_yy/σJ_yrd)+(τ_J/τ_rdJ)**2
...
LongitudinalJ=σJ_xx/σxrd
TransverseJ=σJ_yy/σy_rd
ShearJ=τ_J/τ_rd
BiaxialJ=sqrt(x15)
