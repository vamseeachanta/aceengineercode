...
#  Using default settings
    ...
    customData = {
        ...
        "MassPerUnitLength": data['MainPipe']['equivalentPipe']['massPerUnitLength'] / 1000,
    ...
    customData = {
        ...
        "MassPerUnitLength": data['BuoyPipe']['equivalentPipe']['massPerUnitLength'] / 1000,
    ...
    model2["Lines"] = []
    model2["Lines"].append(lines(data))
...
def lines(data):
    ...
    if data['default']['Analysis']['SLWR']:
        model.update({"Connection, ConnectionX, ConnectionY, ConnectionZ, ConnectionAzm, ConnectionDec, ConnectionGamma, ReleaseStage": \
                [["Vessel1", 80, 32, data['LazyWaveCatenaryDefinition']['HangoffAboveKeel'], 90, 180-data['LazyWaveCatenaryDefinition']['declinationAngle'], 90+data['LazyWaveCatenaryDefinition']['declinationAngle'], "~"], \
                ["Anchored", data['lazyWaveCatenaryResult']['Summary']['HangoffToTDP']['X'] + data['LazyWaveCatenaryDefinition']['TDPToAnchor'] + data['FEASettings']['AnchorAdjustment'] +32, -80, \
                data['MainPipe']['InsulationSection']['OD']*0.0254/2, 0, 90, 0, "~"]]})

    if data['default']['Analysis']['SCR']:
        model.update({"Connection, ConnectionX, ConnectionY, ConnectionZ, ConnectionAzm, ConnectionDec, ConnectionGamma, ReleaseStage": \
                [["Vessel1", 80, 32, data['LazyWaveCatenaryDefinition']['HangoffAboveKeel'], 90, 180-data['LazyWaveCatenaryDefinition']['declinationAngle'], 90+data['LazyWaveCatenaryDefinition']['declinationAngle'], "~"], \
                ["Anchored", data['catenaryResult']['X'] + data['simpleCatenaryDefinition']['TDPToAnchor'] + data['FEASettings']['AnchorAdjustment'] +32, -80, \
                data['MainPipe']['InsulationSection']['OD']*0.0254/2, 0, 90, 0, "~"]]})
    ...
    if data['default']['Analysis']['SLWR']:
        model.update({"LineType, Length, TargetSegmentLength": \
                    ...
                    ["MainPipe", data['FEASettings']['Mesh']['Top']['L']-data['LazyWaveCatenaryDefinition']['TaperJoint']['L'], data['FEASettings']['Mesh']['Top']['Size']*2], \
...
def WriteOrcaflexModel(Files, cfg, FEAType, LoadingIndex = 0):
    with open(cfg['fileName'], 'w') as f:
        for file in Files:
            if isinstance(file, str):
                ...
                yaml.dump(file, f)
    print('Successfully write file: "{0}"' .format(cfg['fileName']))
